...
投資信託として保有しているのはS&P500とオルカンです。他のファンドを幅広く調べてきたわけではありません。この記事では、私が重視する項目に加えて、書類を比較する際の確認方法を整理しています。
:::
...
純資産総額は値上がりだけでも増えるため、その増加を「資金流入」とは判断できません。設定・解約や受益権口数の推移も確認します。月報に流出入がなければ、そこだけで資金流入額を断定しません。
...
次は同じ指数を追う架空の2ファンドの記入例です。運用会社の資料から転記する項目と、判断を分けます。いずれも12か月の同一期間、税込・年率と仮定しています。

| 転記する項目 | ファンドA（架空） | ファンドB（架空） | 確認する書類 |
|---|---:|---:|---|
| 信託報酬 | 0.10% | 0.08% | 目論見書 |
| 総経費率 | 0.13% | 0.15% | 運用報告書 |
| 購入時手数料 | 0% | 0% | 目論見書・販売会社 |
| 対象期間 | 同じ12か月 | 同じ12か月 | 各表の注記 |

信託報酬だけならBが低い一方、この例の総経費率ではAが低くなっています。**信託報酬は総経費率に含まれるので、0.10%と0.13%を足して0.23%とはしません。** また、総経費率には原則として売買委託手数料・有価証券取引税などが含まれません。「1万口当たりの費用明細」も確認します。[金融庁の費用開示資料](https://www.fsa.go.jp/singi/singi_kinyu/market-system/siryou/20211206/01.pdf)でもこの違いが説明されています。

報告期間が半年なら、期間の費用率か年率換算済みかを先に確認します。過去の実費は将来の確定費用ではありません。比較欄の最後に「安い方を選ぶ」だけでなく、「期間が違うので保留」「償還条件を追加確認」など、残った疑問も記載すると判断につながります。
