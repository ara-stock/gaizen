...
1. 楽天証券の口座を開設する
2. 積立の決済方法を決める（楽天銀行との連携は任意）
3. 楽天証券でNISA口座を開設する
4. S&P500に連動する投資信託を選ぶ
5. NISAのつみたて投資枠で、月1,000円の自動積立を設定する

30日は作業を整理する目安で、初回買付までの所要日数を保証するものではありません。口座審査や決済方法の締切により初回買付が翌月以降になる場合は、口座区分・金額・次回予定日まで確認できれば、この段階の作業は完了です。買付結果は実行後に確認します。

楽天証券は筆者が使っている一例です。すでに別の証券会社でNISAを利用している方は、その口座で同じ確認を進められます。楽天銀行やクレジットカードの契約は、NISAを始めるための必須条件ではありません。
...
| 項目 | 目的 |
|---|---|
| 楽天銀行（任意） | 生活資金と投資資金の移動を簡単にする |
| マネーブリッジ（任意） | 楽天銀行と楽天証券を連携する |
| NISA口座 | 対象商品の運用益を非課税にする |
| 楽天カード積立（任意） | 決済方法の一つ。カードを使わない積立方法もある |
...
ただし、S&P500が唯一の正解ではありません。世界各国へ分散するeMAXIS Slim 全世界株式（オール・カントリー）も比較対象になります。両者の違いと私自身の選び方は「[S&P500とオルカン、私が両方を保有する理由](/blog/why-sp500-over-allcountry/)」で整理しています。
...
設定後は、積立予定日を確認します。初回買付前なら、約定履歴が空でも異常とは限りません。[楽天証券のクレカ積立案内](https://www.rakuten-sec.co.jp/web/rfund/guide/creditcard.html)で締切と買付日を確認し、設定照会画面の予定日と照合してください。買付が終わってから約定履歴を見ます。
...
個別株を買う場合は、[GAIZEN FINANCEの投資方針](/about/)で判断軸を確認し、少額から始めます。
...
私は現在、NISAでクレカ積立を利用しています。上の増額例は読者向けの段階例で、私が実際にこの金額の順番で増やした履歴ではありません。
...
- [ ] 月1,000円を投資に回せると判断した
- [ ] 楽天証券を開設した
- [ ] 決済方法を決めた（銀行連携・カードは必要な場合に設定）
- [ ] 楽天証券でNISA口座を開設した
- [ ] NISAつみたて投資枠で投資信託を設定した
- [ ] 初回買付予定日を確認した。買付後は月1,000円の約定を確認する
