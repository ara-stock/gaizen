...
**プリセットの戦略を選ぶだけで、複雑な運用を自動で実行してくれる機能です。**

たとえば「SOLをCollateralにした高利回り戦略」「USDCベースの安定利回り戦略」など、リスク・リターンのプロファイルが異なる複数のストラテジーから選択できます。自分でパラメータを設定する必要がなく、DeFiに慣れていないユーザーでも使いやすい設計になっています。
...
日本では、**暗号資産交換業を行う事業者は金融庁への登録が義務付けられています**（資金決済法）。JupiterのようなDeFiプロトコルは海外のスマートコントラクトであり、現時点では日本の規制当局への登録はありません。

日本の金融庁は、未登録の海外事業者による日本居住者向けのサービス提供について注意喚起を行っています。グレーゾーンであることは間違いなく、参加する場合は自己責任の範囲内での判断が求められます。
...
- DeFiプロトコルの利用自体を直接禁止する法律は現時点では存在しない
- ただし規制の整備が進んでおり、将来的に変わる可能性がある
- 得られた収益は適切に申告する必要がある
