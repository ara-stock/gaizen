...
> **筆者の保有・利益相反:** 筆者はJUPを保有し、Jupiter Earnを利用しています。記事末尾には紹介リンクがあり、利用状況により筆者が特典を受け取る場合があります。
>
> **数値の基準日:** 記事内のAPY・Supply・LTV・清算条件は、筆者が2026年5月に確認した画面と公開データの記録です。現在の条件を示すものではありません。利用前にJupiter公式画面で最新値と契約条件を確認してください。
...
レバレッジがかかる分、担保資産（JUICED等）の価格が下落した際のリスクも比例して拡大します。確認時の画面では清算ペナルティが**1%**と表示されていましたが、現在の条件はポジションを作る前に公式画面で確認してください。
...
フィッシングやソーシャルエンジニアリングは、スマートコントラクトの脆弱性とは別に管理すべき主要リスクです。被害原因の比率を一律に比較することはできないため、プロトコル側と利用者側の両方のリスクを前提にします。
...
DeFi取引の税務上の扱いは、トークン交換、利回りの受領、報酬の確定時点など取引内容によって変わり得ます。収益が課税対象になる可能性を前提に取引履歴を保存し、国税庁の暗号資産に関する案内を確認したうえで、不明点は税理士へ相談してください。
