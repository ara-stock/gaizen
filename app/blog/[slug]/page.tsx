...
export async function generateStaticParams() {
  return getAllPostSlugs('ja', true).map(slug => ({ slug }))
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const hasEnglishVersion = getAllPostSlugs('en', true).includes(slug)
  return {
    ...
    alternates: {
      ...
      languages: {
        ja: `https://gaizen.xyz/blog/${slug}/`,
        ...(hasEnglishVersion && { en: `https://gaizen.xyz/en/blog/${slug}/` }),
      },
    },
    openGraph: {
      title: frontmatter.title,
      description: frontmatter.description,
      type: 'article',
      url: `https://gaizen.xyz/blog/${slug}/`,
...
export default async function ArticlePage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    inLanguage: 'ja',
    url: `https://gaizen.xyz/blog/${slug}/`,
    mainEntityOfPage: `https://gaizen.xyz/blog/${slug}/`,
          ...
          <article className="min-w-0">
            <header className="mb-10 pb-8 border-b" style={{ borderColor: 'var(--border)' }}>
              <div className="flex flex-wrap gap-2 mb-4">
                {frontmatter.tags.slice(0, 3).map(tag => <Tag key={tag} tag={tag} linked={false} />)}
              </div>
            ...
            {nextPost && (
              ...
              >
                <span className="text-xs tracking-widest font-semibold" style={{ color: 'var(--accent)' }}>次に読む</span>
