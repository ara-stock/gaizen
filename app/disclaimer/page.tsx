...
const sections = [
  ...
  {
    title: '投資リスクについて',
    body: `本サイトで取り上げる主な資産クラスには、以下のようなリスクがあります。\n\n【株式・ETF・投資信託】価格変動リスク・為替変動リスク・信用リスク・流動性リスク・カントリーリスク。元本を割り込む可能性があります。\n\n【暗号資産（仮想通貨）】価格変動が非常に大きく、短期間で大幅な損失が生じる可能性があります。また、取引所の破綻・ハッキング・ウォレットの紛失・スマートコントラクトの脆弱性・プロトコル変更・規制変更などのリスクがあります。暗号資産は預金ではなく、預金保険の対象外です。資金決済法などの規制対象であり、デリバティブ等の取引形態によっては金融商品取引法も関係します。利用するサービスごとに適用法令と利用者保護の範囲を確認してください。\n\n【不動産投資信託（REIT）】賃料収入の変動・不動産市況の変化・金利上昇による価格下落リスクがあります。`,
  },
  {
