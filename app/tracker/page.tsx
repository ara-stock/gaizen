...
export const metadata: Metadata = {
  title: '仮想通貨プロジェクト トラッカー',
  description: '筆者が実際に触っている仮想通貨プロジェクトを、TGE時期・ポイント期待値・チーム信頼度・VC調達額・運営拠点で一覧比較。取組中／様子見のステータスは自分用に管理できます。',
  alternates: { canonical: 'https://gaizen.xyz/tracker/' },
}
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
      <div className="mb-10">
        ...
        <p className="text-sm leading-relaxed max-w-3xl" style={{ color: 'var(--muted)' }}>
          筆者が実際に触っている、または参加を検討しているプロジェクトだけを載せています。調達額・チーム・TGE時期は公式発表と報道で確認し、
          ポイント期待値とチーム信頼度は筆者の主観評価（5段階）です。評価の根拠は各プロジェクトの詳細ページに書いています。
        </p>
        <p className="text-xs mt-3" style={{ color: 'var(--muted)' }}>最終更新: {updatedAt}</p>
      </div>

      <TrackerBoard projects={projects} locale="ja" />
      ...
      <section className="mt-12 p-5 rounded-xl border text-xs leading-relaxed" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface)', color: 'var(--muted)' }}>
        ...
        <ul className="list-disc pl-5 space-y-1.5">
          <li>「始める」ボタンは紹介（リファラル）リンクです。登録や取引に応じて筆者が報酬を受け取る場合があります。掲載の有無や評価は報酬と連動させていません。</li>
          <li>エアドロップやポイントの価値は保証されません。TGEが行われない、配布対象から外れる、預けた資金を失う可能性があります。</li>
          <li>海外のDEX・サービスは日本の金融庁に登録されていません。利用可否と税務はご自身で確認してください。</li>
          <li>評価基準の詳細は<Link href="/editorial-policy/" className="underline" style={{ color: 'var(--accent)' }}>編集方針</Link>と<Link href="/disclaimer/" className="underline" style={{ color: 'var(--accent)' }}>免責事項</Link>をご覧ください。</li>
        </ul>
      </section>
    </div>
  )
